...
public class MarqueService implements IDao<Marque>{
	...
	public boolean update(Marque o) {
		String sql = "update marque set codeM = ? , libelle = ? where id  = ?";
        try {
            PreparedStatement ps = Connexion.getInstane().getConnection().prepareStatement(sql);
            ps.setString(1, o.getCodeM());
            ps.setString(2, o.getLibelle());
            
            ps.setInt(3, o.getIdM());
            if (ps.executeUpdate() == 1) {
                return true;
            }
        } catch (SQLException e) {
	...
	public Marque findById(int id) {
		Marque m = null;
	        String sql = "select * from marque where idM  = ?";
	        try {
	            PreparedStatement ps = Connexion.getInstane().getConnection().prepareStatement(sql);
	            ps.setInt(1, id);
	            ResultSet rs = ps.executeQuery();
	            if (rs.next()) {
	                return new Marque(rs.getInt("idM"), rs.getString("codeM"),rs.getString("libelle"));
	            }
	...
	public List<Marque> findAll() {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                marque.add(new Marque(rs.getInt("idM"), rs.getString("codeM"),rs.getString("libelle")));
            }
